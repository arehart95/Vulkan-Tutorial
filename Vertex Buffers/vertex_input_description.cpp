/* Now we are going to replace the hardcoded vertex data in the vertex shader with a vertex
   buffer in memory. This approach creates a CPU visible buffer and uses memcpy to copy the
   vertex data into it directly, after which we will see how to use a staging buffer to copy
   the vertex data to high performance memory. 
   
   First, change the vertex shader to no longer include the vertex data in the code itself. The
   vertex shader takes input from a vertex buffer using the in keyword: 
   ...
   The inPosition and inColor are vertex attributes. They're properties that are specified per-
   vertex in the vertex buffer, just like how we manually specified a position and color per 
   vertex using the two arrays. Don't forget to recompile the vertex shader!
   
   Just like fragColor, the layout(location = x) annonations assign indices to the inputs that
   we can later use to reference them. It is important to know that some types like dvec3 64-bit
   vectors use multiple slots. That means the index after it must be at least two higher.
   
   		layout(location = 0) in dvec3 inPosition;
   		layout(location = 2) in vec3 inColor;
     
	We're moving the vertex data from the shader code to an array in the code of the program.
	Start by including them GLM library which provides linear alegbra related types like vectors
	and matrices.*/
...
// Create a new structure called Vertex with the two attributes that we'll use in the vertex
// shader.
struct Vertex {
	glm::vec2 pos;
	glm::vec3 color;
...
/* Now use the Vertex structure to specify an array of vertex data. This is exactly the same
	position and color values as before, but now they are combined into one array of vertices.
