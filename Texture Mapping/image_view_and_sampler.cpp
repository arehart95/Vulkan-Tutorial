/*	This time we are going to create two more resources that are needed for the graphics pipeline
	to sample an image. The first resource is one we have seen already while working with swap
	chain images, but the second one is new and it relates to how the shader will read texels from
	the image. 
	
	We know that images are accessed through image views rather than directly. We will need to create
...
private:
	...
	// Simplified createImageViews to reflect addition of new function createImageView
    }
	...
	*/
	...
	//	viewInfo.components has been left out because VK_COMPONENT_SWIZZLE_IDENTITY is defined as 0.
		VkImageView imageView;
		if (vkCreateImageView(device, &viewInfo, nullptr, &imageView) != VK_SUCCESS) {
			throw std::runtime_error("failed to create texture image view!"):
		}
	/*	The code for this function is based on createImageViews, the only two changes made are the format
		and the image. This code has already been abstracted to a new function because the logic is
		so identical. createTextureImageView has been updated to reflect this. */
